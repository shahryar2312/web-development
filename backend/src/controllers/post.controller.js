...
const createPost = asyncHandler(async (req, res) => {
  ...
  // 23.04 Ilia Klodin - adding adming functionality - before posting check if the user is banned
...
/**
 * 22.04 Ilia Klodin - added reddit-like functionality
 * @desc    Lock or unlock a post to prevent new comments
 * @route   PATCH /api/posts/:postId/lock
 * @access  Private — hub moderator, hub creator, or admin
 */
const lockPost = asyncHandler(async (req, res) => {
  const { isLocked } = req.body;
  if (typeof isLocked !== 'boolean') {
    return error(res, 'isLocked must be a boolean.', 400);
  }
  ...
  if (!isMod && !isCreator && req.user.role !== 'admin') {
    return error(res, 'Not authorised to lock this post.', 403);
  }
