/**
 * CreatePostPage.jsx — View 6: Create a new post inside a hub
 *
 * Supports all four post types: text, image (URL), link, lfg.
 * Dynamically shows/hides extra fields based on the selected type.
 *
 * Client-side validation:
 *   - Title: required, 3–300 characters
 *   - Content: required for text/lfg posts
 *   - URL: required and valid URL for image/link posts
 *   - Tags: max 5, each max 30 characters
 *   - LFG fields: playersNeeded (1–100), contactInfo required
 *
 * Maps to backend:
 *   POST /api/hubs/:hubId/posts
 *   Body: { title, content, type, url, tags, flair, lfgDetails }
 */
import React, { useState } from 'react';
import { Link, useNavigate, useParams } from 'react-router-dom';
import { useAuth } from '../context/AuthContext';
import './CreatePostPage.css';

/* Hub map for display names (in full integration, fetched from GET /api/hubs/:slug) */
const HUB_NAMES = {
  valorant:    'Valorant',
  minecraft:   'Minecraft',
  fifa25:      'FIFA 25',
  'elden-ring':'Elden Ring',
  csgo2:       'CS2',
};

const PLATFORMS    = ['PC', 'PlayStation', 'Xbox', 'Nintendo Switch', 'Mobile', 'Cross-platform'];
const REGIONS      = ['NA', 'EU', 'Asia', 'OCE', 'SA', 'Global'];
const SKILL_LEVELS = ['Beginner', 'Intermediate', 'Advanced', 'Expert', 'Any'];

function CreatePostPage() {
  const { slug }            = useParams();
  const { isLoggedIn, user } = useAuth();
  const navigate             = useNavigate();

  const hubName = HUB_NAMES[slug] ?? slug;

  /* ---- Redirect guests ---- */
  if (!isLoggedIn) {
    return (
      <div className="container empty-state" style={{ marginTop: '4rem' }}>
        <h2>You must be logged in to post.</h2>
        <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center', marginTop: '1rem' }}>
          <Link to="/login"    className="btn btn-primary">Log In</Link>
          <Link to="/register" className="btn btn-secondary">Register</Link>
        </div>
      </div>
    );
  }

  /* ---- Post type tab ---- */
  const [postType, setPostType] = useState('text'); // 'text' | 'image' | 'link' | 'lfg'

  /* ---- Form state ---- */
  const [formData, setFormData] = useState({
    title:   '',
    content: '',
    url:     '',
    tags:    '',   // comma-separated input
    flair:   '',
    // LFG-specific
    platform:      'PC',
    region:        'EU',
    skillLevel:    'Any',
    gameMode:      '',
    playersNeeded: '2',
    voiceChat:     false,
    schedule:      '',
    requirements:  '',
    contactInfo:   '',
  });

  /* ---- Validation errors ---- */
  const [errors, setErrors] = useState({});

  /* ---- Loading / success state ---- */
  const [submitting, setSubmitting] = useState(false);

  /* ---- Handlers ---- */
  const handleChange = (e) => {
    const { name, value, type, checked } = e.target;
    setFormData((prev) => ({ ...prev, [name]: type === 'checkbox' ? checked : value }));
    setErrors((prev) => ({ ...prev, [name]: '' }));
  };

  /**
   * parseTags — Splits the tag string by comma, trims, dedupes, and validates.
   * Returns { valid: boolean, tags: string[], error: string }
   */
  const parseTags = (raw) => {
    if (!raw.trim()) return { valid: true, tags: [] };
    const tags = [...new Set(raw.split(',').map((t) => t.trim()).filter(Boolean))];
    if (tags.length > 5)          return { valid: false, tags, error: 'Maximum 5 tags allowed.' };
    const tooLong = tags.find((t) => t.length > 30);
    if (tooLong)                  return { valid: false, tags, error: `Tag "${tooLong}" exceeds 30 characters.` };
    return { valid: true, tags };
  };

  /**
   * validate — Full client-side validation.
   * Rules mirror the backend createRules in post.routes.js and Post model.
   */
  const validate = () => {
    const errs = {};

    // Title — required, 3–300 chars
    if (!formData.title.trim()) {
      errs.title = 'Title is required.';
    } else if (formData.title.trim().length < 3) {
      errs.title = 'Title must be at least 3 characters.';
    } else if (formData.title.trim().length > 300) {
      errs.title = 'Title cannot exceed 300 characters.';
    }

    // Content — required for text and lfg types
    if ((postType === 'text' || postType === 'lfg') && !formData.content.trim()) {
      errs.content = 'Post body is required for text and LFG posts.';
    }
    if (formData.content.length > 40000) {
      errs.content = 'Content cannot exceed 40,000 characters.';
    }

    // URL — required and valid for image/link types
    if (postType === 'image' || postType === 'link') {
      if (!formData.url.trim()) {
        errs.url = `URL is required for ${postType} posts.`;
      } else {
        try {
          new URL(formData.url); // throws if invalid
        } catch {
          errs.url = 'Please enter a valid URL (e.g. https://example.com).';
        }
      }
    }

    // Tags
    const { valid: tagsValid, error: tagError } = parseTags(formData.tags);
    if (!tagsValid) errs.tags = tagError;

    // Flair — max 50
    if (formData.flair.length > 50) {
      errs.flair = 'Flair cannot exceed 50 characters.';
    }

    // LFG-specific validation
    if (postType === 'lfg') {
      const pn = parseInt(formData.playersNeeded, 10);
      if (isNaN(pn) || pn < 1)  errs.playersNeeded = 'Players needed must be at least 1.';
      if (pn > 100)              errs.playersNeeded = 'Players needed cannot exceed 100.';
      if (!formData.contactInfo.trim()) errs.contactInfo = 'Contact information is required.';
    }

    setErrors(errs);
    return Object.keys(errs).length === 0;
  };

  /**
   * handleSubmit — Validates and posts the new post.
   * Maps to: POST /api/hubs/:hubId/posts
   */
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!validate()) return;

    setSubmitting(true);

    const { tags } = parseTags(formData.tags);

    /*
     * --- BACKEND INTEGRATION POINT ---
     * First resolve the hub's _id from the slug:
     *   GET /api/hubs?slug=<slug>   (or store from prior fetch)
     *
     * Then submit:
     * const body = {
     *   title:   formData.title.trim(),
     *   content: formData.content.trim(),
     *   type:    postType,
     *   url:     postType === 'image' || postType === 'link' ? formData.url : undefined,
     *   tags,
     *   flair:   formData.flair.trim(),
     *   lfgDetails: postType === 'lfg' ? {
     *     platform:      formData.platform,
     *     region:        formData.region,
     *     skillLevel:    formData.skillLevel,
     *     gameMode:      formData.gameMode,
     *     playersNeeded: parseInt(formData.playersNeeded, 10),
     *     voiceChat:     formData.voiceChat,
     *     schedule:      formData.schedule,
     *     requirements:  formData.requirements,
     *     contactInfo:   formData.contactInfo,
     *     status: 'open',
     *   } : undefined,
     * };
     *
     * const res = await fetch(`/api/hubs/${hub._id}/posts`, {
     *   method: 'POST',
     *   headers: { 'Content-Type': 'application/json' },
     *   credentials: 'include',
     *   body: JSON.stringify(body),
     * });
     * const data = await res.json();
     * if (res.ok) navigate(`/post/${data.post._id}`);
     */

    // Mock: navigate to the hub page with a success message
    setTimeout(() => {
      setSubmitting(false);
      navigate(`/hub/${slug}`);
    }, 800);
  };

  /* Character count helpers */
  const titleLeft   = 300 - formData.title.length;
  const contentLeft = 40000 - formData.content.length;

  return (
    <div className="container">
      <div className="create-post-layout">

        {/* ===== Form area ===== */}
        <section className="create-post__form-area" aria-label="Create post form">

          {/* Breadcrumb */}
          <nav className="create-post__breadcrumb" aria-label="Breadcrumb">
            <Link to="/">Home</Link>
            <span>›</span>
            <Link to={`/hub/${slug}`}>h/{hubName}</Link>
            <span>›</span>
            <span>Create Post</span>
          </nav>

          <h1 className="create-post__title">Create a Post in h/{hubName}</h1>

          {/* Post type tabs */}
          <div className="post-type-tabs" role="tablist" aria-label="Post type">
            {[
              { key: 'text',  label: '📝 Text'  },
              { key: 'image', label: '🖼 Image' },
              { key: 'link',  label: '🔗 Link'  },
              { key: 'lfg',   label: '👥 LFG'   },
            ].map(({ key, label }) => (
              <button
                key={key}
                role="tab"
                aria-selected={postType === key}
                className={`post-type-tab ${postType === key ? 'active' : ''}`}
                onClick={() => { setPostType(key); setErrors({}); }}
              >
                {label}
              </button>
            ))}
          </div>

          {/* Form */}
          <form onSubmit={handleSubmit} noValidate className="create-post__form card" aria-label="New post form">

            {/* ---- Title ---- */}
            <div className="form-group">
              <label className="form-label" htmlFor="cp-title">
                Title * <span className="create-post__char-count">{titleLeft} characters remaining</span>
              </label>
              <input
                id="cp-title"
                type="text"
                name="title"
                className={`form-input ${errors.title ? 'error' : ''}`}
                placeholder="Give your post a title"
                value={formData.title}
                onChange={handleChange}
                maxLength={300}
                aria-required="true"
                aria-describedby={errors.title ? 'cp-title-error' : undefined}
              />
              {errors.title && <span id="cp-title-error" className="field-error" role="alert">{errors.title}</span>}
            </div>

            {/* ---- Content (text + lfg) ---- */}
            {(postType === 'text' || postType === 'lfg') && (
              <div className="form-group">
                <label className="form-label" htmlFor="cp-content">
                  {postType === 'lfg' ? 'Description *' : 'Body *'}
                  <span className="create-post__char-count">{contentLeft.toLocaleString()} characters remaining</span>
                </label>
                <textarea
                  id="cp-content"
                  name="content"
                  className={`form-textarea ${errors.content ? 'error' : ''}`}
                  rows={6}
                  placeholder={postType === 'lfg' ? 'Describe what you are looking for…' : 'Tell your story…'}
                  value={formData.content}
                  onChange={handleChange}
                  maxLength={40000}
                  aria-required="true"
                  aria-describedby={errors.content ? 'cp-content-error' : undefined}
                />
                {errors.content && <span id="cp-content-error" className="field-error" role="alert">{errors.content}</span>}
              </div>
            )}

            {/* ---- URL (image + link) ---- */}
            {(postType === 'image' || postType === 'link') && (
              <div className="form-group">
                <label className="form-label" htmlFor="cp-url">
                  {postType === 'image' ? 'Image URL *' : 'Link URL *'}
                </label>
                <input
                  id="cp-url"
                  type="url"
                  name="url"
                  className={`form-input ${errors.url ? 'error' : ''}`}
                  placeholder="https://example.com/..."
                  value={formData.url}
                  onChange={handleChange}
                  aria-required="true"
                  aria-describedby={errors.url ? 'cp-url-error' : undefined}
                />
                {errors.url && <span id="cp-url-error" className="field-error" role="alert">{errors.url}</span>}
                {postType === 'image' && formData.url && !errors.url && (
                  <div className="create-post__img-preview">
                    <img src={formData.url} alt="Preview" onError={(e) => { e.target.style.display = 'none'; }} />
                  </div>
                )}
              </div>
            )}

            {/* ---- LFG-specific fields ---- */}
            {postType === 'lfg' && (
              <div className="lfg-fields">
                <h3 className="lfg-fields__title">LFG Details</h3>
                <div className="lfg-fields__grid">

                  <div className="form-group">
                    <label className="form-label" htmlFor="cp-platform">Platform</label>
                    <select id="cp-platform" name="platform" className="form-select" value={formData.platform} onChange={handleChange}>
                      {PLATFORMS.map((p) => <option key={p} value={p}>{p}</option>)}
                    </select>
                  </div>

                  <div className="form-group">
                    <label className="form-label" htmlFor="cp-region">Region</label>
                    <select id="cp-region" name="region" className="form-select" value={formData.region} onChange={handleChange}>
                      {REGIONS.map((r) => <option key={r} value={r}>{r}</option>)}
                    </select>
                  </div>

                  <div className="form-group">
                    <label className="form-label" htmlFor="cp-skill">Skill Level</label>
                    <select id="cp-skill" name="skillLevel" className="form-select" value={formData.skillLevel} onChange={handleChange}>
                      {SKILL_LEVELS.map((s) => <option key={s} value={s}>{s}</option>)}
                    </select>
                  </div>

                  <div className="form-group">
                    <label className="form-label" htmlFor="cp-players">Players Needed *</label>
                    <input id="cp-players" type="number" name="playersNeeded" className={`form-input ${errors.playersNeeded ? 'error' : ''}`} min={1} max={100} value={formData.playersNeeded} onChange={handleChange} />
                    {errors.playersNeeded && <span className="field-error" role="alert">{errors.playersNeeded}</span>}
                  </div>

                  <div className="form-group">
                    <label className="form-label" htmlFor="cp-mode">Game Mode</label>
                    <input id="cp-mode" type="text" name="gameMode" className="form-input" placeholder="e.g. Ranked, Casual" value={formData.gameMode} onChange={handleChange} maxLength={100} />
                  </div>

                  <div className="form-group">
                    <label className="form-label" htmlFor="cp-schedule">Schedule</label>
                    <input id="cp-schedule" type="text" name="schedule" className="form-input" placeholder="e.g. Weekday evenings" value={formData.schedule} onChange={handleChange} maxLength={200} />
                  </div>

                  <div className="form-group lfg-fields__full">
                    <label className="form-label" htmlFor="cp-requirements">Requirements</label>
                    <input id="cp-requirements" type="text" name="requirements" className="form-input" placeholder="e.g. Must have mic, Gold+ rank" value={formData.requirements} onChange={handleChange} maxLength={500} />
                  </div>

                  <div className="form-group lfg-fields__full">
                    <label className="form-label" htmlFor="cp-contact">Contact Info *</label>
                    <input id="cp-contact" type="text" name="contactInfo" className={`form-input ${errors.contactInfo ? 'error' : ''}`} placeholder="e.g. Discord: User#1234" value={formData.contactInfo} onChange={handleChange} maxLength={200} aria-required="true" />
                    {errors.contactInfo && <span className="field-error" role="alert">{errors.contactInfo}</span>}
                  </div>

                  <div className="form-group lfg-fields__full">
                    <label className="create-post__checkbox">
                      <input type="checkbox" name="voiceChat" checked={formData.voiceChat} onChange={handleChange} />
                      Voice chat required
                    </label>
                  </div>
                </div>
              </div>
            )}

            {/* ---- Tags ---- */}
            <div className="form-group">
              <label className="form-label" htmlFor="cp-tags">Tags (optional)</label>
              <input
                id="cp-tags"
                type="text"
                name="tags"
                className={`form-input ${errors.tags ? 'error' : ''}`}
                placeholder="e.g. guide, ranked, tips  (comma-separated, max 5)"
                value={formData.tags}
                onChange={handleChange}
                aria-describedby={errors.tags ? 'cp-tags-error' : 'cp-tags-hint'}
              />
              <span id="cp-tags-hint" className="field-hint">Separate with commas. Max 5 tags, 30 chars each.</span>
              {errors.tags && <span id="cp-tags-error" className="field-error" role="alert">{errors.tags}</span>}
            </div>

            {/* ---- Flair ---- */}
            <div className="form-group">
              <label className="form-label" htmlFor="cp-flair">Flair (optional)</label>
              <input
                id="cp-flair"
                type="text"
                name="flair"
                className={`form-input ${errors.flair ? 'error' : ''}`}
                placeholder="e.g. Clip, Discussion, Question"
                value={formData.flair}
                onChange={handleChange}
                maxLength={50}
                aria-describedby={errors.flair ? 'cp-flair-error' : undefined}
              />
              {errors.flair && <span id="cp-flair-error" className="field-error" role="alert">{errors.flair}</span>}
            </div>

            {/* ---- Posting as ---- */}
            <p className="create-post__posting-as">
              Posting as <strong>u/{user?.username}</strong> in <strong>h/{hubName}</strong>
            </p>

            {/* ---- Submit ---- */}
            <div className="create-post__actions">
              <button type="submit" className="btn btn-primary" disabled={submitting} aria-busy={submitting}>
                {submitting ? 'Posting…' : '📢 Post'}
              </button>
              <Link to={`/hub/${slug}`} className="btn btn-secondary">Cancel</Link>
            </div>
          </form>
        </section>

        {/* ===== Tips sidebar ===== */}
        <aside className="create-post__sidebar">
          <div className="sidebar__widget">
            <h2 className="sidebar__title">📋 Posting Tips</h2>
            <ul className="create-post__tips">
              <li>Make the title descriptive and specific</li>
              <li>Use the correct post type</li>
              <li>Add relevant tags to help people find your post</li>
              <li>Be respectful and follow hub rules</li>
              <li>For LFG, include your contact info and schedule</li>
            </ul>
          </div>
          <div className="sidebar__widget">
            <h2 className="sidebar__title">h/{hubName} Rules</h2>
            <p style={{ fontSize: 'var(--fs-sm)', color: 'var(--color-text-muted)' }}>
              Please review the hub rules before posting.
            </p>
            <Link to={`/hub/${slug}`} className="btn btn-secondary sidebar__more" style={{ marginTop: '0.75rem' }}>
              View Hub Rules
            </Link>
          </div>
        </aside>

      </div>
    </div>
  );
}

export default CreatePostPage;
