...
function HubPage() {
  ...
  const handleJoinToggle = () => {
     ...
     * fetch(`/api/hubs/${hub._id}/join`, { method, credentials: 'include' });
     */
    setJoined((prev) => !prev);
  ...
  const sortedPosts = [...posts].sort((a, b) => {
    ...
    return (b.voteScore * 0.7 + new Date(b.createdAt).getTime() * 0.3) - // 28.04 Ilia Klodin - fixed the sorting formula.
           (a.voteScore * 0.7 + new Date(a.createdAt).getTime() * 0.3); // previous formula multiplied both sides by the same amount, which did nothing.
  });
